...
export default function ProfilePage() {
  ...
  // Put a claimed item back on the board. Owner-only, enforced inside the RPC.
  const handleUnclaim = async (item: Item) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        throw new Error(messages[result.error || ""] || t("profile.unclaim.generic"));
      }
    } catch (err) {
      setActionError(
        err instanceof Error ? err.message : t("profile.unclaim.fallback")
      );
    } finally {
    ...
    <div className="max-w-[860px] mx-auto px-6 py-12">
      ...
      {showSuccess && (
        <div className="mb-6 p-4 rounded-xl bg-teal/10 border border-teal text-center">
          <span className="text-teal font-semibold">{"\u2713"} {t("profile.submitSuccess")}</span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="py-16 text-center flex flex-col items-center gap-3">
          <h3 className="font-display text-lg font-extrabold">
            {t("profile.empty.titleA")} {type} {t("profile.empty.titleB")}
          </h3>
          <p className="text-sm text-white/35 max-w-[280px] leading-relaxed">
            {t("profile.empty.bodyA")} {type} {t("profile.empty.bodyB")}
          </p>
          <Link
          ...
          >
            {type === "found" ? t("profile.empty.ctaFound") : t("profile.empty.ctaLost")}
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <ConfirmModal
        ...
        confirmText={t("profile.delete.confirm")}
        variant="danger"
        loading={deleting}
      />
